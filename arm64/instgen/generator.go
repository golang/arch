...
func Generate(insts []*xmlspec.InstructionParsed, outputDir string, genE2E bool) {
	...
	if genE2E {
		sort.Slice(e2eTestData, func(i, j int) bool {
			return e2eTestData[i].GoOp < e2eTestData[j].GoOp
		})
		sort.Slice(e2eErrorData, func(i, j int) bool {
			return e2eErrorData[i].GoOp < e2eErrorData[j].GoOp
		})
...
func constructInstance(enc *xmlspec.EncodingParsed) (*e2eData, *e2eData) {
	...
	for range 100 {
		...
		if name == "CMPLT" || name == "CMPLE" || name == "CMPLO" || name == "CMPLS" {
			...
			todoCase := e2eData{GoOp: enc.GoOp[1:], Asm: fmt.Sprintf("// TODO: %s", name)}
	...
	if enc.Asm == "BFMMLA  <Zda>.H, <Zn>.H, <Zm>.H" ||
		...
		todoCase := e2eData{GoOp: enc.GoOp[1:], Asm: fmt.Sprintf("// TODO: %s", enc.Asm)}
		return &todoCase, &todoCase
	}
